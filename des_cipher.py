...
def function_f(right_bits, round_key):
...
def des_block(block_bits, round_keys):
    ...
    for i in range(16):
        old_L = L
        L = R
        R = xor(old_L, function_f(R, round_keys[i]))
...
def encrypt(plaintext, key):
    ...
    for i in range(0, len(bits), 64):
        block = bits[i:i+64]
        encrypted_bits += des_block(block, round_keys)

    hex_result = hex(int(encrypted_bits, 2))[2:].upper()

    return hex_result

def decrypt(cipher_hex, key):
    encrypted_bits = bin(int(cipher_hex, 16))[2:]

    while len(encrypted_bits) % 64 != 0:
    ...
    for i in range(0, len(encrypted_bits), 64):
        ...
        decrypted_bits += des_block(block, round_keys)
